...
//    UIEdgeInsets inset = scrollView.contentOffset;
    ...
    DDLog(@"日历 scrollView offX == %f offY == %f  class == %@",offX,offY,scrollView.class);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    DDLog(@"scrollView class == %@",scrollView);
    self.currentScrollView = scrollView;
    if ([scrollView class] == [LTSCalendarSelectedWeekView class]) {
        DDLog(@"周历滚动了");
...
- (NSDate *)getNewCurrentData{
    ...
    dayComponent.day   = 0;
    //周历滚动
    if ([self.currentScrollView class] == [LTSCalendarSelectedWeekView class]) {
         dayComponent.weekOfYear += 1 ;
         dayComponent.month = currentPage - (NUMBER_PAGES_LOADED / 2);
    }
    //月历滚动
